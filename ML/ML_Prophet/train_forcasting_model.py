...
def print_header():
    """Print script header."""
    print("\n" + "="*70)
    print("🤖 Prophet Forecasting - Model Training")
    print("="*70 + "\n")
...
def fetch_metric_data(prom, metric_name, query, hours=1):
    ...
    try:
        ...
        if len(df) < 500:
            ...
            print(f"\n⚠️  Limited data: {len(df)} points (~{data_hours:.1f}h). Prophet needs 2+ weeks for full seasonality.")
            print("   Using conservative model settings (no seasonality, 3 changepoints).")
...
def train_prophet_model(df, metric_name, metric_config, metric_key):
    ...
    try:
        ...
        print(f"   Stats: mean={data_mean:.2f}, std={data_std:.2f}, range=[{data_min:.2f}, {data_max:.2f}], trend={trend:+.2f}")
        print(f"   Growth: {'logistic (0-100%)' if is_percentage else 'linear'}")
